...
export default {
  ...
  computed: {
    ...
    reversedKeys() {
      ...
      return Object.keys(this.product.bestellungen).reverse();
    }else{
      return false;
    }
    },
    ...
    //     for (let customerKey in this.kunden) {
    //       const customer = this.kunden[customerKey];
    //       const customerOrders = customer.bestellungen;
    //       for (let orderId in customerOrders) {
    //         const order = customerOrders[orderId];
    //         const orderArticles = order.artikel;
    //         const orderBestellung = order.bestellung;
    //         if (Object.values(orderArticles).includes("-NJPqZzscyjts2y_yEkO")) {
    //           for (let hh in this.product.bestellungen){
    //             console.log(hh)
    //           test.push({ orderBestellung, orderId });
    //         }
    //         }
    //       }
    //     }
